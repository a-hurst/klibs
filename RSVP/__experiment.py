...
class RSVP(klibs.Experiment):
	frame_rate = 0.16  # ms
	relative_box_width = 1.3  # degrees of visual angle
	...
	def box_surface(self, char=None, cue_box=False):
		draw_context = aggdraw.Draw("RGBA", (self.box_bounds, self.box_bounds), Params.default_fill_color)
...
app = RSVP('rsvp_replication').run()
# app = RSVP('rsvp_replication').db.export()
