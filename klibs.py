...
def swat(desc = None, subj = None ):
	if mute:
		return None
	###just for debugging, a quick 'got here' with optional details
	global swatcount
	swatcount += 1
	if desc == None or (desc and subj == None):
...
class KlibBase(object):
	...
	def eAttr(self, argName, given, expected, kw = True):
		if kw:
			return "The keyword argument, '{0}', was expected to be of type '{1}' but '{2}' was given.".format(argName, type(given), type(expected))
		else:
			return "The argument, '{0}', was expected to be of type '{1}' but '{2}' was given.".format(argName, type(given), type(expected))
	...
	def prettyJoin(self, array, whitespace=1, delimiter="'", delimitBehavior=None, prepend=None, beforeLast=None, eachN=None, afterFirst=None, append=None):
			...
			if eachN is (list or tuple):
				if len(eachN) == 2:
					if type(eachN[0]) is int:
						...
						self.log("Klib.prettyJoin() config parameter 'eachN[0]' must be an int, '{0}' {1} passed. {2}".format(eachN, type(eachN, 10)))
				else:
					raise ValueError("Klib.prettyJoin() config parameter 'eachN' must be a tuple or list of length 2, but {0} of length '{1}' passed. {2}".format(type(eachN),len(eachN), msg))
			elif eachN is not None:
				raise TypeError("Klib.prettyJoin() config parameter 'eachN' must be a list or tuple, '{0}' passed. {1}".format(type(eachN), msg))
...
class App(KlibBase):
	...
	def __trial_func(self, *args, **kwargs):
		...
		# try:
		self.TRIAL_NUM += 1
		self.trialPrep(*args, **kwargs)
		trialData = self.trial(*args, **kwargs)
		# except:
		# 	raise
		# finally:
		self.__logTrial(trialData)
		self.trialCleanUp()

	def __experiment_func(self, *args, **kwargs):
		"""
		...
		for i in range(self.practiceBlocks):
			self.__generateTrials(practice=True, codeGen=self._codeFunc, **self._EXP_FACTORS)
		...
		for i in range(self.blocks):
			...
			self.__generateTrials(practice=False, codeGen=self._codeFunc, **self._EXP_FACTORS)
			if self.trialsPerBlock % len(self.trials):
				e = "The desired number of trials in the block, {0}, is not a multiple of the minimum number of trials, {1}.".format(self.trialsPerBlock, len(self.trials))
				raise ValueError(e)
			else:
				self.trials *= (self.trialsPerBlock/len(self.trials))
				random.shuffle(self.trials) # ROSS: SHOULD I HAVE DONE THIS? IT DOESN'T OTHERWISE SEEM THAT TRIALS ARE SHUFFLED?
	...
	def __init__(self,  el = None, paramsDir = None, paramFile = None):
		KlibBase.__init__(self)
		# intialize params from param file
		if paramFile:
			self.__appParams(paramFile, paramsDir)

		# initialize pygame, screen surface and screen parameters
		pygame.init()
		self.__screenInit(self.VIEW_DISTANCE, flags = self.SCRNFLGS)
		...
		if el:
			...
			self.EL = EyeLink()
		self.noTracker = self.EL.dummyMode
		self.EL.screenSize = self.screenxy

	def __appParams(self, pfile, path = None):
		# imports all vars from parameters file and assigns them to the current App instance
		if path:
			modContents = __import__(path, globals(), locals(), [pfile], -1)
			modContents = getattr(modContents, pfile)
			params = dir(modContents)
		else:
			modContents = __import__(pfile)
			params = dir(modContents)

		for p in params:
			if p[:2] != "__":
				ucp = p.upper()
				val = getattr(modContents, p)
				setattr(self, ucp, val)

	def __initDB(self, dbName):
		self.db = Database(dbName, ('schema', True))
	...
	def __generateTrials(self, practice=False, codeGen=None, **kwargs):
		"""
		Example usage:
		...
		for e in evalQueue:
			...
			for t in trials:
				t.append(eval('t[factors.index(\''+operands[0]+'\')]'+operator+'t[factors.index(\''+operands[1]+'\')]'))
	...
	def __logTrial(self, trialData, autoID = True):
		if autoID:
	...
	def __screenInit(self, viewDistance, flags = None):
		if flags:
		...
		e = "viewRule must be int or a tuple/list containing [int,str]; '{0}' of type '{1}' passed.".format(repr(viewDistance), type(viewDistance))
	...
	def alert(self, alertString, urgent=False, displayFor=0):
		...
		text = self.message(alertString, color=(255,0,0), location=self.screenc, registration=5, fontSize=self.text._defaultFontSize * 2, rtrn = True)
	...
	def bliterate(self, targ, registration = 7, pos=[0,0], destination = None, flags = None, area = None ):
		...
		if type(pos) is tuple or type(pos) is list:
			...
			e = "Value of parameter 'pos' was neither a valid string nor a list or tupple of length 2; value '{0}' of type '{1}' passed.".format(repr(pos), type(pos))
			raise ValueError(e)
		if area:
			if flagstr:
	...
	def blockBreak(self, message=None, isPath=False):
		"""
		...
		:raise:
		"""
		default = "You've completed block {0} of {1}. When you're ready to continue, press any key.".format(self.BLOCK_NUM, self.blocks)
		if isPath:
			try:
				...
				if pathExists:
					...
					e ="'isPath' parameter was True but '{0}' was not a valid path. Using default message".format(message)
					raise IOError(e)
			except IOError as e:
	...
	def boundedBy(self, pos, left, right, top, bottom):
		xpos = int(pos[0])
		ypos = int(pos[1])
		...
		if all(type(val) is int for val in (left, right, top, bottom)) and type(pos) is tuple:
			...
			raise TypeError("One or more arguments of boundedBy() were of the wrong type; 'pos' must be a tuple and the remaining arguments must be integers.")
	...
	def getDemographics(self):
		...
		if self.db.isUnique(name, 'userhash', 'participants'): #names must be unique; returns True if unique, False otherwise
			...
			if not self.participantId:
				raise ValueError("A problem was encountered when retrieving participant_id and it could not be set. Exiting program.")
		else:
	...
	def expSchema(self, blocks, trialsPerBlock, practiceBlocks, trialsPerPracticeBlock=None):
	...
	def instructions(self, text, isPath = False):
		if isPath:
			if type(text) is str:
				...
				e = "App.instructions() requires the param 'text' to be of type 'str' but '{0}' was passed.".format(type(text))
				raise TypeError(e)

		self.sf()
		self.message(text, location= "center", wrapWidth=800)

	def listen(self, maxWait, keyMap=None, hostFunc = None, hostArgs=None, hostKWArgs=None, elArgs=None, nullResponse = None, timeOutMessage=None, responseCount=None, responseMap=None, interrupt=True, quickReturn = False):
	...
	# @canonical
	def message(self, message, font=None, fontSize=None, color=None, bgcolor=None, bgIsKey=None, location=None, registration=None, wrap=None, wrapWidth=None, delimiter=None, rtrn=False, flip = False, fullscreen=False):
		renderConfig = {}
		...
		if self.isTuplist(location):
			if len(location) == 2:
				...
				raise AttributeError("Coordinate locations must be either a tuple or list of length 2; '{0}' passed".format(repr(location)))
		elif location == None:
			...
			raise ValueError("The location '{0}' could not be interpreted by the App, please use a tuple/list of coordinates or else a relative keyword (ie. 'center')".format(repr(location)))
	...
	def pause(self):
		...
		while self.PAUSED:
			self.message('PAUSED', fullscreen=True, location='center', fontSize=96, color=(255, 0, 0), registration=5, rtrn ='flip')
			self.overWatch()
			self.flip()

	def preBlit(self, targ, startTime, endTime, registration=7, pos=[0, 0], destination=None, flags=None, area=None, func=None):
		"""
		Blit to the screen buffer, wait until endTime to flip. Check func often if set.
		:type startTime: float
		:param targ:
		:param startTime: Time trial began (from time.time())
		:param endTime: The time post trial after which the screen should be flipped.
		:param registration:
		:param pos:
		:param destination:
		:param flags:
		:param area:
		:param func: A function called repeatedly until the duration has passed. Don't make it long.
		"""
	...
	def preBug(self):
		pass
		# will be a screen that's shown before anything happens in the program to quickly tweak debug settings

	def query(self, query=None, password=False, font=None, fontSize=None, color=None, locations={'query': None, 'input': None}, registration=5, returnType=None, accepted=None):
		...
		if locations is not None:
			if locations.get('query') is None or locations.get('input') is None:
				if self.text.defaultLocations.get('query') is not None and self.text.defaultLocations.get('input') is not None:
		...
		if query is not None:
			...
			querySurface = self.text.renderText(self.text.defaultQueryString, font=font, fontSize=fontSize, color=color, bgcolor=None)
		else:
		...
		if accepted is not None:
			if not self.isTuplist(accepted):
				raise TypeError("The set of accepted responses must be a tuple or a list but type '{0}' was given.".format(type(accepted)))
	...
	@abc.abstractmethod
	def trialPrep(self, trialInfo, trialNumber):
		return

	def run(self, *args, **kwargs):
		self.__experiment_func(*args, **kwargs)
...
class Database(KlibBase):
			...
			if table != "sqlite_sequence":
				...
				for col in columns:
					if col[2] in ('text', 'TEXT'):
						...
						colType = 'unknown'
						e =  "column '{0}' of table '{1}' has type '{2}' on the database but was assigned a type of 'unknown' during schema building'".format(col[1], table,  col[2])
	...
	def rebuild(self):
		...
		if self.schemaSQL is not None:
			if self.__deploySqlSchema(self.schemaSQL, False):
				initialized = True
			else:
				self.__dropTables(self.tableList, True)
				raise IOError(e)
		elif self.sqlSchemaPath is not None:
			if self.__deploySqlSchema(self.sqlSchemaPath, True):
				initialized = True
			else:
				self.__dropTables(self.tableList, True)
				raise IOError(e)
	...
	def isUnique(self, value, field, table):
	...
	def insert(self, data=None, table=None, tidyExec=True):
		...
		if data.__class__.__name__ == 'EntryTemplate':
			...
			template = None
			if table:
				if not self.defaultTable:
					raise AttributeError("Either provide a table when calling insert() or set a defaultTable with App.Database.setDefaultTable().")
				else:
					table = self.defaultTable
				template = self.tableSchemas[table]
			if not template:
				raise AttributeError("The supplied table name, '{0}' was not found in Database.tableSchemas".format(table))
			...
			if len(data) == fieldCount:
				...
				raise AttributeError('Length of data list does not much number of table columns. You are collecting more data items than exist in the database table.')
...
class EntryTemplate(KlibBase):
	def __init__(self, tableName, tableSchema, instanceName):
		...
		try:
			...
			if not self.name:
				raise AttributeError('InstanceName could not be set, ensure parameter is passed during initialization and is a string.')
		except AttributeError as e:
	...
	def buildQuery(self, queryType):
		...
		for fieldName in self.schema:
			...
			columnOrder = fieldParams['order'] # the data order and schema order may differ, which is fatal; this index is for (re)correlating
			...
			if self.data[columnOrder] == 'null':
				if fieldParams['allowNull'] :
					...
					e = "[instance '{0}']: The required fieldParams '{1}' had a null value.".format(self.name,fieldName)
					raise IndexError(e)
		...
		if queryType == 'insert':
			try:
				...
				print e
				e = "[instance '{0}'] SQL query string couldn't be written because NoneType items were found in either 'insertTemplate' or 'EntryTemplate.data'; printing debug info.".format(self.name)
				debug = "\t****DEBUGGING INFO:\n\t\tinsertTemplate = {0}\n\t\tself.data = {1}".format(repr(insertTemplate), repr(self.data))
	...
	def log(self, field, value):
		# TODO: Add some basic large for making type conversions where possible (ie. if expecting an float
		...
		if (valueType == fieldType):
			...
			e = "Schema for this table expected the field '{0}' type '{1}', but the passed value ('{2}') was of type '{3}'.".format(field, self.schema[field]['type'], value, type(value).__name__)
			raise TypeError(e)
...
class EyeLink(pylink.EyeLink):
	...
	def trackerInit(self):
		...
		self.setOfflineMode()
		self.sendCommand("screen_pixel_coords = 0 0 {0} {1}".format(self.screenSize[0], self.screenSize[1]))
		self.sendMessage("link_event_filter = SACCADE")
		self.sendMessage("link_event_data = SACCADE")
		self.sendMessage("DISPLAY_COORDS 0 0 {0} {1}".format(self.screenSize[0], self.screenSize[1]))
...
class KeyMap(KlibBase):
	...
	def addKeys(self, keyNames, keyCodes, keyVals):
		if type(keyNames) is str and type(keyCodes) is int and type(keyVals) in (int, str):
			...
			e = "One argument passed to addKey was of the wrong type; keyName was '{0}', keyCode was '{1}' and keyVal was '{2}'.".format(type(keyNames), type(keyCodes), type(keyVals))
			raise TypeError(e)
...
class TextLayer(KlibBase):
	...
	def renderText(self, string, font=None, fontSize=None, color=None, bgcolor=None):
		...
		try:
			...
			raise GenWarning("Call to renderText() failed; string, when rendered, is too large for screen. Setting wrap to True and trying again...")
	...
	def wrappedText(self, text, delimiter=None, fontSize=None, font=None, wrapWidth=None):
		...
		try:
			if wrapWidth is not None:
				if type(wrapWidth) not in [int, float]:
					raise ValueError("The config option 'wrapWidth' must be an int or a float; '{0}' was passed. Defaulting to 80% of app width.".format(repr(wrapWidth)))
					...
					raise ValueError("A wrapWidth of '{0}' was passed which is either too big to fit inside the app or else is negative (and must be positive). Defaulting to 80% of app width.")
		...
		except ValueError("'{0}' was passed as a delimiter, should be a simple string that won't appear by accident (ie.'\\n')".format(repr(delimiter))) as e:
