...
def get_clicks(button=None, released=False, queue=None):
	...
	If a button is specified, this function will only return the pixel coordinates for clicks
	of that button. Otherwise, this function will return click coordinates for all buttons.
	Valid button names are ``'left'``, ``'right'``, and ``'middle'``. If an event queue is not
	provided, :func:`~klibs.KLEventQueue.pump` will be called and the returned event queue will
	be used.

	Args:
		button (str, optional): The name of the button to check for clicks. Defaults to ``None``
			(returns clicks from all buttons).
		released (bool, optional): If True, this function will return the coordinates for mouse
			button release events instead of mouse button click events. Defaults to False.
		queue (:obj:`List` of :obj:`sdl2.SDL_Event`, optional): A list of events to check
			for valid mouse button events.

	Returns:
		:obj:`List`: A list containing the (x, y) coordinates of any matching mouse click events.
